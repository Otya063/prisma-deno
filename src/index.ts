...
import { PrismaAccelerate, ResultError } from 'npm:prisma-accelerate-local@0.2.7/lib';

const queryEngineWasmFileBytes: Promise<ArrayBuffer> = (async () =>
    (await fetch(new URL('../../node_modules/@prisma/client/runtime/query-engine.wasm', import.meta.url))
        .then((r) => r.arrayBuffer())
        .catch(() => undefined)) ?? (await fetch(new URL('../node_modules/@prisma/client/runtime/query-engine.wasm', import.meta.url)).then((r) => r.arrayBuffer())))();
...
export const createServer = ({ secret }: { https?: { cert: string; key: string }; secret: string }) => {
